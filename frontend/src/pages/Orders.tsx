...
interface Order {
  ...
  order_type: 'BUY' | 'SELL';
  target_price: number;
  status: 'PENDING' | 'COMPLETED' | 'CANCELLED' | 'EXECUTED';
  created_at: string;
}
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (response.status === 404) {
        // No orders found — treat as empty list
        setOrders([]);
      } else {
      ...
      if (axios.isAxiosError(err)) {
        toast.error(err.response?.data?.error || err.response?.data?.msg || 'Failed to fetch orders');
      } else {
